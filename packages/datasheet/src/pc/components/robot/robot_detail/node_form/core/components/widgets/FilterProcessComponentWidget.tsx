...
interface IFilterProcessComponentWidgetProps extends IWidgetProps {
  processCode?: string;
}

export const FilterProcessComponentWidget = ({ value, onChange, processCode}: IFilterProcessComponentWidgetProps) => {
  const style = { width: '100%' };
  const [options, setOptions] = useState([]);

  useEffect(() => {
    if (options.length == 0 && processCode && processCode != '') {
      Api.loadDingtalkProcessComponents(processCode).then((res) => {
        const { data } = res;
        const processList = data.data;
        const optionList: any = [];
        processList.forEach(element => {
          const option = {label: element.label, value: element.label};
